...
df =pd.read_excel("./Catastro/TRABAJOCATASTRO.xlsx",sheet_name="PRIMERLOTE",header=0)
...
contadorCoincidencias=0
tokenizer =nltk.tokenize.TreebankWordTokenizer()
for indice in tqdm(range(int(len(df['CT_Fraccio'])))):
    text=str(df.at[df.index[contcel],'CT_Fraccio'])
    tokens=tokenizer.tokenize(text)
